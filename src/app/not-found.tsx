...
export default function RootNotFound() {
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center bg-bg-main text-charcoal relative overflow-hidden">
      <div
      ...
      <section className="relative z-10 w-full max-w-2xl mx-auto px-6 sm:px-8 py-16 sm:py-20 text-center">
        <div className="relative mb-8 sm:mb-10">
          <span
            className="block text-[10rem] sm:text-[14rem] font-bold leading-none tracking-tighter text-charcoal/10 select-none"
            aria-hidden
          >
            404
          </span>
        ...
        <p className="inline-flex items-center gap-2 rounded-full border border-border-light bg-bg-soft px-4 py-1.5 text-xs font-medium uppercase tracking-[0.2em] text-muted mb-6">
          <span className="h-1.5 w-1.5 rounded-full bg-accent-red" />
          Page not found
        </p>

        <h1 className="text-2xl sm:text-3xl md:text-4xl font-semibold tracking-tight text-charcoal mb-4">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          <Link
            href={`/${defaultLocale}`}
            className="inline-flex items-center justify-center gap-2 rounded-full bg-accent-red px-6 py-3.5 text-sm font-semibold text-white shadow-lg hover:opacity-90 transition-all"
          >
            Back to home
          </Link>
        </div>
